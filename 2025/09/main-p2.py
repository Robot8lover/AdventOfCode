...
def do_case(inp: str, sample=False):
    ...
    x_coords = sorted(set(v[0] for v in tiles))
    y_coords = sorted(set(v[1] for v in tiles))

    x_coord_dict = {v: i for i, v in enumerate(x_coords)}
    y_coord_dict = {v: i for i, v in enumerate(y_coords)}

    comp_tiles = [(x_coord_dict[t[0]], y_coord_dict[t[1]]) for t in tiles]

    walls = [[False] * len(x_coords) for _ in range(len(y_coords))]
    grid = [[False] * len(x_coords) for _ in range(len(y_coords))]
    ...
    for row_index in range(len(y_coords)):
        inside = False
        wall_row = walls[row_index]
        grid_row = grid[row_index]
        on_wall = False
        for col_index in range(len(x_coords)):
    ...
    area = 0
    for i, p in enumerate(comp_tiles):
        for p2 in comp_tiles[:i]:
            start_x, end_x = min_max((p[0], p2[0]))
            start_y, end_y = min_max((p[1], p2[1]))
            temp_area = (x_coords[end_x] - x_coords[start_x] + 1) * (y_coords[end_y] - y_coords[start_y] + 1)
            if temp_area > area:
                filled = True
