...
def do_case(inp: str, sample=False):
    ...
    for line in lines:
        ...
        target_lights = [1 if c == "#" else 0 for c in groups[0]]
        ...
        max_voltage = max(target_voltages)
        ...
        x in Z^n
        maximize [-1 -1 -1 -1 ...] * x (or minimize [1 1 1 1 1 ..] * x)
        subject to wiring_vecs^T*x + s = target_voltages^T (alternatively wiring_vecs^T*x <= target_voltages^T)
        s >= 0
        x >= 0
        """
        ...
        target_voltages_vec_array = np.array(target_voltages)
        bounds = optimize.Bounds(0, max_voltage)
        integrality = np.full_like(function_coefficients, True)
        constraints = optimize.LinearConstraint(A=wiring_vecs_array, lb=target_voltages_vec_array)

        result = milp(c=function_coefficients, constraints=constraints, integrality=integrality, bounds=bounds)

        sprint(result.success)
        sprint(result.x)
        sprint(result.fun)
