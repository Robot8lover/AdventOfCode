...
def make_grid(*dimensions: int, fill=None):
...
def last_index(l, value, start=None, stop=None, /):
    # remember that start and stop are converted so may feel backwards
    def convert_index(i):
        return len(l) - 1 - i
    if start is None and stop is None:
        return convert_index(l[::-1].index(value))
    elif start is not None and stop is None:
        return convert_index(l[::-1].index(value, convert_index(start)))
    else:
        return convert_index(l[::-1].index(value, convert_index(start), convert_index(stop)))
...
def lremove(l, indices):
    if is_number(indices):
        # tbh, only really want int
        indices = [indices]
...
def lmask(n: int, l: int):
    # which is really right from the integer perspective but left from the list perspective
    return n & ((1 << l) - 1)

def rmask(n: int, l: int, bit_length: int):
    # which is really left from the integer perspective but right from the list perspective
    return n & (((1 << l) - 1) << (bit_length - l))
...
def is_number(x):
    return isinstance(x, (int, float, complex)) and not isinstance(x, bool) # may not work for Numpy objects
